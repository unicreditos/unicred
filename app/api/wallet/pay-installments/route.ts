...
export async function POST(req: Request) {
  try {
    ...
    const ids = Array.isArray((body as any).installmentIds) ? (body as any).installmentIds.map(String) : []; return mobileJson(req, await mobileWalletPayInstallments(userId, ids))
  } catch (err) {
