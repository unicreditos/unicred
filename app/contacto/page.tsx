      ...
      <div className="grid gap-6 lg:grid-cols-5">
          ...
          <PageSection eyebrow="Oficina Central" title="Dirección y horarios de atención">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="flex flex-col gap-3 rounded-2xl border border-border/60 bg-background p-5">
                ...
                <p className="text-sm leading-relaxed text-brand-navy-700">{BRAND.address}</p>
                <ul className="space-y-1.5 text-sm text-brand-navy-700">
                  <li className="flex items-center gap-2"><Clock className="h-4 w-4 text-muted-foreground" /> Lunes a Viernes · 9:00 a 18:00 hs (atención presencial con cita)</li>
                  <li className="flex items-center gap-2"><Globe2 className="h-4 w-4 text-muted-foreground" /> {BRAND.domain} · {BRAND.legalName}</li>
                </ul>
              </div>
              <div className="flex flex-col gap-3 rounded-2xl border border-border/60 bg-background p-5">
                ...
                <p className="text-sm leading-relaxed text-brand-navy-700">Operamos 100% digital en todo el país. La atención presencial es solo en CABA, con cita previa. No publicamos sucursales que no operan al público.</p>
                <ul className="space-y-1.5 text-sm text-brand-navy-700">
                  <li className="flex items-center gap-2"><Clock className="h-4 w-4 text-muted-foreground" /> Soporte remoto: lunes a viernes 9 a 18 hs</li>
                  <li className="flex items-center gap-2"><Globe2 className="h-4 w-4 text-muted-foreground" /> Incidentes de seguridad: {BRAND.supportEmail}</li>
                </ul>
              </div>
            </div>
          </PageSection>
        ...
        <aside className="space-y-6 lg:col-span-2">
          <PageSection eyebrow="Canales" title="Formulario y email">
            <Grid cols={2}>
              <a href="#formulario" className="flex h-full flex-col gap-3 rounded-2xl border border-brand-primary/20 bg-brand-primary/5 p-5 transition hover:bg-brand-primary/10">
                <MessageCircle className="h-6 w-6 text-brand-primary" />
                <h3 className="text-base font-semibold text-brand-navy">Mensaje web</h3>
                <p className="text-sm text-muted-foreground">El envío llega a soporte si el servidor de correo está configurado.</p>
                <span className="mt-auto text-xs font-semibold text-brand-primary">Ir al formulario →</span>
              </a>
              <a href={`mailto:${BRAND.supportEmail}`} className="flex h-full flex-col gap-3 rounded-2xl border border-border/60 bg-background p-5 transition hover:border-brand-primary/30">
                <Send className="h-6 w-6 text-brand-primary" />
                <h3 className="text-base font-semibold text-brand-navy">Email soporte</h3>
              ...
              <a href={`mailto:${BRAND.privacyEmail}`} className="flex h-full flex-col gap-3 rounded-2xl border border-border/60 bg-background p-5 transition hover:border-brand-primary/30">
                <Handshake className="h-6 w-6 text-brand-navy-500" />
                <h3 className="text-base font-semibold text-brand-navy">Datos personales</h3>
                <p className="text-sm text-muted-foreground">{BRAND.privacyEmail}</p>
                <span className="mt-auto text-xs font-semibold text-brand-navy-700">ARCO →</span>
              </a>
              <a href={`mailto:${BRAND.merchantsEmail}`} className="flex h-full flex-col gap-3 rounded-2xl border border-border/60 bg-background p-5 transition hover:border-brand-primary/30">
                <PhoneCall className="h-6 w-6 text-brand-primary" />
                <h3 className="text-base font-semibold text-brand-navy">Comercios</h3>
          ...
          <PageSection eyebrow="Equipo comercial" title="Adhesiones PyMEs y Comercios">
            <div className="rounded-2xl border border-border/60 bg-muted/40 p-5">
              <p className="text-sm text-brand-navy-700">Si sos dueño/a de comercio o PyME y querés un asesor comercial que te acompañe en la adhesión y capacitación de tu equipo:</p>
